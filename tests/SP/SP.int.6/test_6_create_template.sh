...
result=$(curl -i -X GET http://int-sp-ath.5gtango.eu:32002/api/v3/services)
...
json=$(curl -i -H "Accept: application/json" -H "Content-Type: application/json" -X GET http://int-sp-ath.5gtango.eu:5998/api/nst/v1/descriptors/${nst_uuid})
echo $json

#STEP 6: Delete the NST 
json=$(curl -X DELETE http://int-sp-ath.5gtango.eu:5998/api/nst/v1/descriptors/{$nst_uuid})
echo "-------------------"
echo $json
